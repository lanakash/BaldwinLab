# -*- coding: utf-8 -*-
"""
Created on Thu Aug 24 15:39:57 2017
...
EnrichDate = 'August 14'    #input('Enter string of enrichment batch desired with no space ei. "May16":')

#####MAKE FILE LOCATION MORE GENERAL?############
#Files = g.glob('C:\Users\Lana\Desktop\Nitrate Analysis\*'+ EnrichDate)

Folders = g.glob('C:\Users\Lana\Desktop\Nitrate Analysis\*'+ EnrichDate +'*\*')
...
for k in files.index:
    ...
    stddev = np.sqrt(stddev/(len(stdlist)-1))
    ...
    for a in Samp.index:
        ...
        if files.loc[k,'Type'] == 'Nitrate+Nitrite':
            ...
            dataplot.loc[5*files.loc[k,'Day']  + i,'Nit+Nit_Error'] = dataplot.loc[i,'Nit+Nit']*np.sqrt((Samp.loc[a,'Error']/Samp.loc[a,' Ordinate (A)'])**2+(np.sqrt(len(y)-1)*p['a'].stderr/p['a'].value)**2)     
        else:
            dataplot.loc[5*files.loc[k,'Day']  + i,'Nitrite'] = 1000*Samp.loc[a,' Ordinate (A)']/p['a'].value
            dataplot.loc[5*files.loc[k,'Day']  + i,'Nitrite_Error'] = dataplot.loc[i,'Nitrite']*np.sqrt((Samp.loc[a,'Error']/Samp.loc[a,' Ordinate (A)'])**2+(np.sqrt(len(y)-1)*p['a'].stderr/p['a'].value)**2)
        i += 1
