# -*- coding: utf-8 -*-
"""
Created on Thu Aug 10 16:16:42 2017
...
EnrichDate = 'August 8'    #input('Enter string of enrichment batch desired with no space ei. "May16":')
...
for k in files.index:
    ...
    Samp = Samp.drop(0)
    Samp['Error'] = np.array([0.1]*len(Samp.index))
    ...
    num_points = 1000
    x_fit = np.linspace(min(x)-max(x)/10, max(x) + max(x)/10, num=num_points)
    y_fit = model(p, x_fit)
    ...
    plt.figure(0)
    #############GIVE ME A TITLE#################
    plt.subplot(2,np.ceil(len(Folders)/2) ,n)
    ...
    plt.axis([min(x)-max(x)/10, max(x)+ max(x)/10, -0.1, 1])
    plt.show()
    ...
    for a in Samp.index:
        ...
        if files.loc[k,'Type'] == 'Nitrate+Nitrite':
            dataplot.loc[5*files.loc[k,'Day']  + i,'Nit+Nit'] = 100*(Samp.loc[a,' Ordinate (A)']-p['b'].value)/p['a'].value
            dataplot.loc[5*files.loc[k,'Day']  + i,'Nit+Nit_Error'] = 100*dataplot.loc[i,'Nit+Nit']*np.sqrt((np.sqrt(Samp.loc[a,'Error']**2+p['b'].stderr**2)/(Samp.loc[a,' Ordinate (A)']-p['b'].value))**2+(p['a'].stderr/p['a'].value)**2)    
        else:
            dataplot.loc[5*files.loc[k,'Day']  + i,'Nitrite'] = 1000*(Samp.loc[a,' Ordinate (A)']-p['b'].value)/p['a'].value
            dataplot.loc[5*files.loc[k,'Day']  + i,'Nitrite_Error'] = 1000*dataplot.loc[i,'Nitrite']*np.sqrt((np.sqrt(Samp.loc[a,'Error']**2+p['b'].stderr**2)/(Samp.loc[a,' Ordinate (A)']-p['b'].value))**2+(p['a'].stderr/p['a'].value)**2)
        i += 1
...
for samp in nameList:
    ...
    plt.plot(xy['Day'], xy['Nitrate'], '-k')
    #plt.errorbar(xy['Day'], xy['Nitrate'], yerr=xy['Nitrate_Error'], fmt='.', color='k')
    plt.axis([-0.1, max(files['Day'])+0.1, min(dataplot['Nitrate'])-max(dataplot['Nitrate'])/20, max(dataplot['Nitrate'])+max(dataplot['Nitrate'])/20])   
...
for samp in nameList:
    ...
    plt.plot(xy['Day'], xy['Nitrite'], '-k')
    #plt.errorbar(xy['Day'], xy['Nitrite'], yerr=xy['Nitrite_Error'], fmt='.', color='k')
    plt.axis([-0.1, max(files['Day'])+0.1, min(dataplot['Nitrite'])-max(dataplot['Nitrite'])/20, max(dataplot['Nitrite'])+max(dataplot['Nitrite'])/20])    
    i += 1        
for samp in nameList:
    xy = dataplot[dataplot['Name'] == samp]
    xy = xy.sort_values('Day')
    ...
    plt.plot(xy['Day'], xy['Nit+Nit'], '-k')
    #plt.errorbar(xy['Day'], xy['Nit+Nit'], yerr=xy['Nit+Nit_Error'], fmt='.', color='k')
    plt.axis([-0.1, max(files['Day'])+0.1, min(dataplot['Nit+Nit'])-max(dataplot['Nit+Nit'])/20, max(dataplot['Nit+Nit'])+max(dataplot['Nit+Nit'])/20])   
    plt.xlabel(r'Day')
    i += 1
