# -*- coding: utf-8 -*-
"""
Created on Wed Dec 20 17:57:16 2017
...
EnrichDate = 'October 2'    #input('Enter string of enrichment batch desired with no space ei. "May16":')
...
for k in files.index:
    ...
    preSamp = preSamp[~preSamp['Type'].str.contains('zinc')]
    Samp['Name'] = preSamp['Sample']
    Samp['Abs'] = preSamp['Abs']
    print Samp
    ...
    for a in Samp.index:
        ...
        if files.loc[k,'Type'] == 'Nitrate+Nitrite':
            ...
            if Samp.loc[a,'Abs'] == 0:
                ...
                dataplot.loc[len(Samp)*files.loc[k,'Day']  + i,'Nit+Nit_Error'] = 0#dataplot.loc[len(Samp)*files.loc[k,'Day']  + i,'Nit+Nit']*np.sqrt((Samp.loc[a,'Error']/Samp.loc[a,'Abs'])**2+(np.sqrt(len(y)-1)*p['a'].stderr/p['a'].value)**2)/np.sqrt(replicate-1)          
        else:
            ...
            for r in dataplot.index:
                if files.loc[k,'Day'] == dataplot.loc[r, 'Day'] and Samp.loc[a,'Name'] == dataplot.loc[r, 'Name']:
                    ...
                    if Samp.loc[a,'Abs'] == 0:
                        ...
                        dataplot.loc[r,'Nitrite_Error'] = 0#dataplot.loc[r,'Nitrite']*np.sqrt((Samp.loc[a,'Error']/Samp.loc[a,'Abs'])**2+(np.sqrt(len(y)-1)*p['a'].stderr/p['a'].value)**2)/np.sqrt(replicate-1)
...
for k in dataplot.index:
    if '0mmA' in dataplot.loc[k,'Name']:
        dataplot.loc[k,'Name'] = '0mMA'
    if '0mmB' in dataplot.loc[k,'Name']:
        dataplot.loc[k,'Name'] = '0mMB'
    if '0mmC' in dataplot.loc[k,'Name']:
        dataplot.loc[k,'Name'] = '0mMC'        
########################################
#########################################################################################
...
for samp in nameList:
    ...
    plt.subplot(3,len(nameList)/3, i)
