# -*- coding: utf-8 -*-
"""
Created on Wed Nov 22 11:40:39 2017
...
LiveData = countData[countData['Name'].str.contains('live')]
for i in LiveData.index:
    LiveData.loc[i,'Name'] = LiveData.loc[i,'Name'].split('live')[0]
...
for i in LiveData.index:
    for a in countData.index:   
        if LiveData.loc[i,'Name'] == countData.loc[a,'Name'] and LiveData.loc[i,'Day'] == countData.loc[a,'Day']:
            LiveData.loc[i, 'Countlive'] = countData.loc[a,'Count']-LiveData.loc[i, 'Count']
            LiveData.loc[i, 'Countdead'] = LiveData.loc[i, 'Count']
            LiveData.loc[i, 'Count'] = countData.loc[a,'Count']

nameList = list(set(LiveData['Name']))
nameList.sort()
i = 1
#
#MaxData1 = LiveData[LiveData['Dilution'] == 1]
...
#    
#    plotData = LiveData[LiveData['Name'] == samp]
#    plotData1 = plotData[plotData['Dilution'] == 1]
#    ax = plt.subplot(1,len(nameList), i)
#    plt.plot(plotData1['Day'], plotData1['Countlive']/50*1000, color = 'k')
#    plt.plot(plotData1['Day'], plotData1['Countlive']/50*1000, 'ok')
...
#countData = countData[~countData['Name'].str.contains('test')]
#countData = countData[~countData['Name'].str.contains('Test')]
#countData = countData[~countData['Name'].str.contains('sip')]
#countData = countData[~countData['Name'].str.contains('Sip')]
...
Max1 = max(LiveData['Countlive'])/50*1000
MaxDay1 = max(LiveData['Day'])
Max10 = max(LiveData['Count'])/50*1000
MaxDay10 = max(LiveData['Day'])
Max100 = max(LiveData['Countdead'])/50*1000
MaxDay100 = max(LiveData['Day'])
...
for samp in nameList:
    ...
    plotData10 = plotData[plotData['Dilution'] == 1]
    ax = plt.subplot(3,len(nameList), i+len(nameList))
    plt.plot(plotData10['Day'], plotData10['Count']/50*1000, color = 'k')
    plt.plot(plotData10['Day'], plotData10['Count']/50*1000, 'ok')
    plt.axis([0, MaxDay10, 0, Max10+(Max10/10)])
    plt.xticks(fontsize = 10)
    plt.yticks(fontsize = 10)    
    ...
    plotData100 = plotData[plotData['Dilution'] == 1]
    ax = plt.subplot(3,len(nameList), i+len(nameList)*2)
    plt.plot(plotData100['Day'], plotData100['Countdead']/50*1000, color = 'k')
    plt.plot(plotData100['Day'], plotData100['Countdead']/50*1000, 'ok')
...
fig.text(0.98, 0.8, 'Live', ha='right', va='top', rotation=-90)
fig.text(0.98, 0.4875, 'Live+Dead', ha='right', va='center', rotation=-90)
fig.text(0.98, 0.165, 'Dead', ha='right', va='bottom', rotation=-90)
